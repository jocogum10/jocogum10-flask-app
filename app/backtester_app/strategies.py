...
class CrossUpStrategy():
    ...
    def __init__(self, stock_selected):
        ...
        self.image_link = self.make_stock_image(self.df, self.buy, self.sell)
    ...
    def make_stock_image(self, df_clean, Buy, Sell):
        fig, axs = plt.subplots(figsize=(8,5))
        axs.plot(df_clean['Last'], label='Last Price', c='blue', alpha=0.5)
        axs.plot(df_clean['MA 20'],label='MA 20', c='black', alpha=0.9)
        axs.plot(df_clean['MA 50'],label='MA 50', c='magenta', alpha=0.9)
        axs.scatter(df_clean.iloc[Buy].index, df_clean.iloc[Buy]['Last'], marker='^', color='g', s=100)
        axs.scatter(df_clean.iloc[Sell].index, df_clean.iloc[Sell]['Last'], marker='v', color='r', s=100)
...
class RsiStrategy():
    """RSI cross up strategy"""
    def __init__(self, stock_selected):
        self.df = stock_selected
        self.entry_strat = ['%K crosses up %D']
        self.exit_strat = ['%K crosses down %D']
        self.buy = []
        self.sell = []
        self.backtest()
        self.gain = calculate_gain(self.buy, self.sell, self.df)
    ...
    def backtest(self):
        bought = False
        for i in range(len(self.df)):
            if self.df['%K'].iloc[i] > self.df['%D'].iloc[i] and not bought:
                self.buy.append(i)
                bought = True
            elif self.df['%K'].iloc[i] < self.df['%D'].iloc[i] and bought:
                self.sell.append(i)
                bought = False
